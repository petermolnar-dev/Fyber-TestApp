...
- (void)testOfferCreation {
    ...
    PMOOffer *offerFromFactory = [PMOOfferFactory createOfferFromDisctionary:offerData];
    
    BOOL isOfferSetProperly = [offerFromFactory.offer_id intValue] == [offerData[@"offer_id"] intValue];
    isOfferSetProperly &= [offerFromFactory.title isEqualToString:offerData[@"title"]];
    isOfferSetProperly &= [offerFromFactory.teaser isEqualToString:offerData[@"teaser"]];
    isOfferSetProperly &= offerFromFactory.payout == offerData[@"payout"];
    isOfferSetProperly &= [[offerFromFactory.imageURL absoluteString] isEqualToString:offerData[@"thumbnail"][@"hires"]];
    
    XCTAssertTrue(isOfferSetProperly);
}
